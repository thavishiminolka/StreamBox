...
const getStatus = (item: MovieItem): { label: string; style: any } => {
  ...
  if (release && release > today) return { label: 'Upcoming', style: homeStyles.statusUpcoming };
  if ((item.vote_count || 0) > 1000) return { label: 'Popular', style: homeStyles.statusPopular };
  return { label: 'Active', style: homeStyles.statusActive };
};

export const MovieCard: React.FC<Props> = ({ item, onPress }) => {
  const status = getStatus(item);
  const overview = item.overview ? item.overview.slice(0, 120) + (item.overview.length > 120 ? '…' : '') : 'No overview available.';
  const imageUri = item.poster_path ? `https://image.tmdb.org/t/p/w342${item.poster_path}` : undefined;
  ...
  return (
    ...
    >
      {imageUri ? (
        ...
        <View style={[homeStyles.poster, { alignItems: 'center', justifyContent: 'center' }]}>
